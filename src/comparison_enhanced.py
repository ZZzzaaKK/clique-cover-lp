...
class ComparisonAnalyzer:
    ...
    def perform_statistical_analysis(self, df: pd.DataFrame) -> dict:
        ...
        if "vertex_clique_cover_number" in df.columns:
            cluster_algos = [
                a for a in df["algorithm"].unique() if self.is_cluster_editing_algo(a)
            ]
            ...
            if cluster_algos:
                ...
                for algo in cluster_algos:
                    ...
                    valid = algo_df.dropna(
                        subset=["predicted", "vertex_clique_cover_number"]
                    )
                    ...
                    if len(valid) > 0:
                        ...
                        # Calculate ratios
                        ratios = [
                            c / t if t != 0 else np.nan
                            for c, t in zip(C_vals, theta_vals)
                        ]
            ...
            for i, algo1 in enumerate(algorithms):
                for algo2 in algorithms[i + 1 :]:
                    if "file" in df.columns:
                        ...
                        paired_data2 = []
                        for file in files:
                            d1 = df[(df["algorithm"] == algo1) & (df["file"] == file)][
                                "time"
                            ].values
                            d2 = df[(df["algorithm"] == algo2) & (df["file"] == file)][
                                "time"
                            ].values
                        ...
                        if len(paired_data1) > 1:
                            try:
                                ...
                                analysis["pairwise_comparisons"][
                                    ...
                                    "mean_time_diff": np.mean(
                                        np.array(paired_data1) - np.array(paired_data2)
                                    ),
                                }
                            except:
                                pass
                ...
                if len(algo_data) > 2:
                    ...
                    if len(valid_data) > 2:
                        corr, p_val = stats.pearsonr(
                            valid_data["vertices"], valid_data["time"]
                        )
                    ...
                    if "density" in df.columns:
                        ...
                        if len(valid_data) > 2:
                            corr, p_val = stats.pearsonr(
                                valid_data["density"], valid_data["time"]
                            )
    ...
    def generate_plots(self, df: pd.DataFrame, output_dir: str):
        ...
        if "deviation" in df.columns:
            ...
            if not valid_dev.empty:
                sns.boxplot(
                    data=valid_dev, x="algorithm", y="deviation", palette="Set2"
                )
                plt.title("Deviation from Actual Clique Cover by Algorithm")
                plt.xlabel("Algorithm")
                plt.ylabel("Deviation")
                plt.axhline(
                    y=0, color="green", linestyle="--", alpha=0.5, label="Perfect"
                )
                plt.xticks(rotation=45, ha="right")
                plt.legend()
                plt.tight_layout()
                plt.savefig(
                    os.path.join(output_dir, "deviation_comparison.png"), dpi=150
                )
                plt.close()
        ...
        if "density" in df.columns and df["density"].notna().any():
            ...
            for algo in sorted(df["algorithm"].unique()):
                algo_data = df[
                    (df["algorithm"] == algo) & (df["density"].notna())
                ].copy()
                if algo_data.empty:
                    continue
        ...
        if "clique_number" in df.columns and df["clique_number"].notna().any():
            ...
            for algo in sorted(df["algorithm"].unique()):
                ...
                plt.plot(
                    grouped["clique_number"], grouped["mean"], label=algo, linewidth=2
                )
                plt.fill_between(
        ...
        if "correct" in df.columns and "vertices" in df.columns:
            ...
            accuracy_by_size = (
                df_copy.groupby(["algorithm", "vertex_bin"])["correct"]
                .mean()
                .reset_index()
            )
    ...
    def generate_enhanced_plots(self, df: pd.DataFrame, analysis: dict):
        ...
        if "correct" in df.columns:
            ...
            bars = ax2.bar(
                range(len(correctness_data)),
                correctness_data.values,
                color="lightgreen",
                edgecolor="black",
            )
        ...
        if "vertices" in df.columns and df["vertices"].notna().any():
            for algo in sorted(df["algorithm"].unique()):
                ...
                if len(grouped) > 0:
                    ax3.plot(
                        grouped["vertices"], grouped["mean"], label=algo, linewidth=2
                    )
        ...
        if "deviation" in df.columns and df["deviation"].notna().any():
            valid_dev = df[df["deviation"].notna()]
            sns.boxplot(
                data=valid_dev, x="algorithm", y="deviation", palette="Set2", ax=ax4
            )
        ...
        if "pairwise_comparisons" in analysis and analysis["pairwise_comparisons"]:
            ...
            bars = ax7.bar(
                range(len(p_values)), p_values, color="skyblue", edgecolor="black"
            )
        ...
        if "correct" in df.columns:
            efficiency_data = df.groupby("algorithm").agg(
                {"correct": "mean", "time": "mean"}
            )
            efficiency_data["efficiency"] = (
                efficiency_data["correct"] / efficiency_data["time"]
            )
...
"""
        ...
        if not df.empty:
            ...
            if "deviation" in df.columns and df["deviation"].notna().any():
                min_deviation = (
                    df.groupby("algorithm")["deviation"].mean().abs().idxmin()
                )
    ...
    def generate_markdown_report(self, df: pd.DataFrame, analysis: dict):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Algorithms compared**: {', '.join(df['algorithm'].unique())}\n"
            )
            f.write(
                f"- **Number of test graphs**: {analysis['basic_stats']['n_graphs']}\n"
            )
            f.write(
                f"- **Total comparisons**: {analysis['basic_stats']['total_results']}\n\n"
            )
            ...
            if not df.empty:
                ...
                f.write(
                    f"1. **Fastest Algorithm**: {fastest_algo} (avg: {fastest_time:.4f}s)\n"
                )
                ...
                if "correct" in df.columns:
                    ...
                    f.write(
                        f"2. **Most Accurate**: {most_accurate} ({accuracy * 100:.1f}% correct)\n"
                    )
                ...
                if "deviation" in df.columns and df["deviation"].notna().any():
                    ...
                    f.write(
                        f"3. **Lowest Deviation**: {min_dev_algo} (avg: {min_dev:.2f})\n"
                    )
            ...
            if "wp4_analysis" in analysis:
                ...
                for algo, wp4_data in analysis["wp4_analysis"].items():
                    ...
                    f.write(f"**{algo}:**\n\n")
                    f.write(
                        f"1. **Methods produce similar results**: C(G) ≈ θ(G) with ratio {mean_ratio:.2f}\n"
                    )
                    ...
                    if not np.isnan(p_val):
                        if p_val < 0.05:
                            ...
                            f.write(
                                f"2. **No statistically significant difference**: Wilcoxon test p = {p_val:.3f}\n"
                            )

                    if not np.isnan(corr):
                        f.write(
                            f"3. **Strong correlation**: θ and C are highly correlated (r = {corr:.3f})\n"
                        )
            ...
            if "pairwise_comparisons" in analysis and analysis["pairwise_comparisons"]:
                f.write("\n## Statistical Significance\n\n")
                f.write(
                    "| Comparison | Test | p-value | Significant | Mean Time Diff |\n"
                )
                f.write(
                    "|------------|------|---------|-------------|----------------|\n"
                )
            ...
            if (
                "wp4_analysis" in analysis
                and "vertex_clique_cover_number" in df.columns
            ):
                cluster_algos = [
                    a
                    for a in df["algorithm"].unique()
                    if self.is_cluster_editing_algo(a)
                ]
                ...
                if cluster_algos:
                    ...
                    for algo in cluster_algos:
                        ...
                        size_stats["ratio"] = (
                            size_stats["predicted"]
                            / size_stats["vertex_clique_cover_number"]
                        )

                        f.write(
                            "| Size Category | Mean θ | Mean C | Mean C/θ | Count |\n"
                        )
                        f.write(
                            "|---------------|--------|--------|----------|-------|\n"
                        )
                        ...
                        if (
                            "density" in valid.columns
                            and valid["density"].notna().any()
                        ):
                            ...
                            density_stats = (
                                valid.groupby("density_category")
                                .agg(
                                    {
                                        "vertex_clique_cover_number": "mean",
                                        "predicted": "mean",
                            ...
                            density_stats["ratio"] = (
                                density_stats["predicted"]
                                / density_stats["vertex_clique_cover_number"]
                            )

                            f.write(
                                "| Density Category | Mean θ | Mean C | Mean C/θ | Count |\n"
                            )
                            f.write(
                                "|------------------|--------|--------|----------|-------|\n"
                            )
            ...
            f.write(
                "| Algorithm | Mean Time (s) | Std Time | Min Time | Max Time | Correctness | Avg Deviation |\n"
            )
            f.write(
                "|-----------|---------------|----------|----------|----------|-------------|---------------|\n"
            )
            ...
            if "correlations" in analysis and analysis["correlations"]:
                ...
                for metric, results in analysis["correlations"].items():
                    if results and "correlation" in results:
                        f.write(
                            f"| {metric} | {results['correlation']:.3f} | {results['p_value']:.4f} |\n"
                        )
            ...
            if "vertices" in df.columns:
                ...
                size_perf = (
                    df_copy.groupby(["size_category", "algorithm"])["time"]
                    .mean()
            ...
            f.write("\n## Output Files\n\n")
            f.write(
                f"- Comprehensive analysis plot: `comprehensive_analysis_{self.timestamp}.png`\n"
            )
            f.write(f"- This report: `comparison_report_{self.timestamp}.md`\n")
            f.write("- Raw comparison data: Available in DataFrame format\n")
...
def main():
    ...
    for results_file in results_files:
        ...
        if not graph_directory:
            print("  Warning: Could not determine graph directory, skipping...")
            continue
