...
def _ensure_plots_dir():
    out_dir = os.path.join(
        ...
        "../results/analyses/reductions_visual_plots",
    )
...
def draw_graph(
    G, title, pos=None, highlight_removed=None, highlight_added=None, fig_key=""
):
    """
...
def visualize_before_after(G_before, G_after, label):
    ...
    draw_graph(
        G_after, f"{label} — AFTER", pos=pos, highlight_added=added, fig_key="after"
    )
...
class TestCrownReduction(unittest.TestCase):
    ...
    def test_crown_reduction(self):
        ...
        G_reduced, changed, removed_or_sets, VCC_addition = apply_crown_reduction(
            self.G.copy()
        )
        ...
        self.assertTrue(
            len(set(self.G.nodes()) - set(G_reduced.nodes())) > 0,
